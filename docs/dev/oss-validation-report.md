...
|---|---|---|---|
| Unused-dependency FP rate (YOK002) | < 5% | **0.0%** (0 FP / 2 reported) | ✅ PASS |
| Unused-dependency recall (YOK002 tp) | all detected | **2/2** detected | ✅ PASS |
...
sentinels** — fixtures with a deliberately-unused dependency that yokei must
keep flagging, labelled `tp` in the ground truth:

| Sentinel | Expected YOK002 | Guards |
|---|---|---|
...
of YOK002, so recall is anchored on the deterministic in-repo sentinels.
