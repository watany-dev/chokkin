...
| CHK001–CHK010 issue 判定 | Step 10–12 |
...
- comments — `# chokkin: ignore[CHK003]` 等（§18）
...
| `files[].context` | `FileNode.context`（Step 10 CHK005 入力） |
...
| `lockfile_graph` | Step 7 / CHK004 入力（Phase 0 では graph に未投入可） |
...
| P8 | `# chokkin: ignore[CHK003]` コメント | comment 保持 |
