...
src/
  ...
  graph/
    ...
    build.rs         # build_graph_skeleton(root, config, manifest, sources)
    error.rs         # GraphError
  parser/
    mod.rs
...
fn pipeline_phase0_spike() {
    ...
    for file in sources.python_files() {
        let parsed = parse_file(&root, &file.path, config.target_version)?;
        let file_id = graph.file_id(&file.path).unwrap();
...
| `src/sources/`, `src/manifest/` | 入力 API 確定済み |

### Phase 2: 品質評価（100点満点）

| カテゴリ | 配点 | 得点 | 所見 |
| --- | ---: | ---: | --- |
| モジュール / struct 設計 | 20 | 18 | graph / parser 分離。ID intern 明示 |
| 静的解析制約 | 20 | 20 | Python 非実行を維持 |
| ルール / ポリシー | 20 | 17 | issue 判定は Step 10–12。spike は import のみ |
| エラー処理 | 20 | 18 | 構文エラーは diagnostic 継続 |
| テスト容易性 | 20 | 18 | fixture 10 + 統合 1 本を具体化 |
| **合計** | **100** | **91** | **合格**（90 以上） |
...
| Phase 0 exit（§17） | 部分達成 — CLI 縦スライスは並行 PR |
