...
（§17）の「空 project と小規模 sample project で `uvx yokei` が動く」を満たす縦スライス。
...
| 成果物 | `probe_project(...) -> Result<ProbeReport, ProbeError>` + CLI 統合（`yokei` / `yokei --version` / `yokei --help`） |
...
Config  : pyproject.toml [tool.yokei]
...
fn main() -> ExitCode {
    ...
    if args.version { println!("yokei {}", yokei::VERSION); return ...; }
...
- [ ] `yokei`（引数なし）が空 fixture で exit 0 し、Project / Manifest / Sources を表示する
- [ ] `yokei --help` / `yokei --version` が動作する
