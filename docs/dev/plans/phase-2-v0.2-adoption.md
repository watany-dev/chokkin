...
| v0.2.0 | 導入支援版 | plugin 拡充、schema 文書化、migration notes | OSS gate 合格、CI 導入事例、破壊的変更なし |

v0.2.0 まで JSON schema は draft 扱いを維持する。stable schema は Phase 3 で凍結する。
...
3. manifest/config scan cache — input fingerprint/key/record envelope と disk backend は初期実装済み (`ScanInputFingerprints`, `ScanCacheKey`, `ScanCacheRecord`, `read_scan_record`, `write_scan_record`); result payload は未実装
...
3. Sphinx / MkDocs / Alembic — conventional config/entry extraction is initial implementation (`docs/conf.py`, `mkdocs.yml`, `alembic/env.py`)
...
| baseline | 実運用可能 | schema migration 方針を追加 |
