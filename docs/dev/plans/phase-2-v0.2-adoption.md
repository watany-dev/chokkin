...
2026-06-15 に Rust 1.93 で v0.2 release validation を実測した。
`make check`、`scripts/run-oss-fixture.sh --build`、20-project clone 後の
`make oss-metrics ARGS=--gate`、`make bench`、`make bench-save BASELINE=main`、
`make bench-cmp BASELINE=main` は合格。10k warm cache は Criterion median
で 186.85–204.21 ms。baseline CI 導入事例は chokkin repo 自身の
dogfood job (`.github/workflows/ci.yml` の `chokkin-baseline`) と
checked-in `chokkin-baseline.json` で記録した。
詳細は `docs/dev/v0.2-release-validation.md`。
...
4. warm benchmark fixture と `make bench` comparison — parse cache warm benchmark は初期実装済み (`benches/cache.rs`; 100 / 1k / 5k / 10k files)。2026-06-15 実測では 10k warm cache が 2s 目標を十分に下回った (`docs/dev/v0.2-release-validation.md`)。
