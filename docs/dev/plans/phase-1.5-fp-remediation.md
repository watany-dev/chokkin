...
§17 exit criteria の **CHK002 誤検知率 5% 未満** を通すための横断是正設計。
Phase 1（Steps 1–13 + CLI）と OSS 検証ハーネス（`make oss-metrics`）は完了済み。
2026-06-14 計測では **CHK002 FP 率 100% (155/155)**。crash 0・cold 2s は合格済み。
...
> - [`step-10-dependency-reconciliation.md`](./step-10-dependency-reconciliation.md) — CHK002 判定

## 1. 目的

| 項目 | 内容 |
| --- | --- |
| 解決する問題 | 「実際には使われている依存」を chokkin が利用と結べず CHK002 が全件 FP になる |
...
| exit criteria | CHK002 FP < 5%、未分類 0、crash 0、cold medium ≤ 2s |
...
CHK002 は context を無視して **全宣言を一律 unused 判定**しているため、dev group のみの依存も FP になる。
...
| Workstream | 内容 | FP 寄与 |
| --- | --- | ---: |
| **A. Binary + config usage** | dev tool の CLI 利用を `BinaryUsage` として抽出し CHK002 used 集合へ | ~110 |
| **B. Dev context policy + PDM/Hatch 読取** | dev/test/docs group の CHK002 抑制、`[tool.pdm]` / `[tool.hatch]` 依存抽出 | ~34 の一部 |
...
| 項目 | 理由 |
| --- | --- |
| CHK003 gate | §17 gate 対象外。改善は副次効果として計測のみ |
...
#### 4.B.1 CHK002 dev 抑制ポリシー（§10, §17）

```text
declaration_bucket(dep) == Dev
  AND used に含まれない
  AND --strict でない
    → CHK002 を発行しない（または confidence=Likely + severity=Info に降格）

--strict
    → 現行どおり error（dev 未使用も報告）
```

`detect_unused_dependencies` に `config: &ChokkinConfig` と declaration context を渡し、
`declaration_bucket` + `strict` で skip 判定する。

**production モード:** `--production` 時は dev group を manifest から除外するのではなく、
CHK002 対象から除外（既存 `production` フラグと整合）。
...
`collect_optional_imports`（CHK003 用）は維持。optional でも **宣言があれば used**、
...
| --- | --- | --- |
| `binary-tool-pyproject/` | `[tool.mypy]` / `[tool.ruff]` → CHK002 なし | A |
...
| **P1** | dev group 読取済みなのに CHK002 が一律判定 | §4.B.1 で `detect_unused_dependencies` 修正 |
