...
| 後続ステップへの入力 | Step 3 (manifest extraction) 以降の全ステップが参照する `ChokkinConfig` と、キャッシュ hash 用の `ConfigSources` |

## 2. スコープ

### In scope

- project root 直下の設定ソースを探索し、§5 の `[tool.chokkin]` 相当を読み込む
- 設定が無い場合は **hardcoded defaults** のみで `LoadedConfig` を返す（zero-config）
- 複数ソースの **マージ規則** を固定し、どのファイルが寄与したかを記録する
- `[tool.chokkin.workspaces.<id>]` の読み込み（明示 workspace override）
...
| `chokkin --init` の雛形生成 | v0.1 CLI 機能（`toml_edit` 使用） |
| baseline file (`chokkin-baseline.json`) | v0.2 |
...
§5: `pyproject.toml` の `[tool.chokkin]` を第一候補とし、`chokkin.toml` / `.chokkin.toml` も許容する。

**マージ順（低 → 高。後勝ちで上書き）:**

```text
1. DEFAULTS                    # コード内定数
2. <root>/.chokkin.toml          # 存在すれば
3. <root>/chokkin.toml           # 存在すれば
4. <root>/pyproject.toml の [tool.chokkin]   # 存在すれば（最高優先）
```
...
3. **TOML のみ静的パース** — `pyproject.toml` も文字列として読み、`[tool.chokkin]` サブツリーのみ deserialize する（`setup.py` 実行禁止）
4. **衝突はマージ** — 同一キーが複数ソースにある場合、優先順位の高いソースが勝つ。配列・map は **置換**（deep merge しない）。
   **例外**: `plugins` と `dependencies` は **キー単位マージ**（部分指定しても defaults / 低優先ソースの他キーが残る。`celery = true` だけ書いても pytest 等の既定有効は維持される）
...
| 読み取り対象 | Step 2 の扱い |
| --- | --- |
| `[tool.chokkin.workspaces.<id>]` | 完全に読み込み、`WorkspaceOverride` として保持 |
...
src/
  ...
  config/
    ...
    types.rs          # ChokkinConfig, LoadedConfig, EntrySpec, ...
...
/// Known chokkin plugins (§5, §9). v0.1 enables pytest/django/fastapi by default.
...
pub struct ChokkinConfig {
    ...
    pub ignore: BTreeMap<String, Vec<String>>,  // CHK001..CHK010 -> patterns
...
`apply_overrides(config: &mut ChokkinConfig, overrides: &RuntimeOverrides)` は Step 2 で **単体テスト可能な pure 関数**として実装する。
...
`UnknownKey` は `serde(deny_unknown_fields)` 相当でトップレベル `[tool.chokkin]` に適用。`[tool.chokkin.ignore]` の rule code は `CHK001`–`CHK010` 形式を検証するが、未知 code は **warning ではなく Validation エラー**（typo 早期検出）。
...
pub use types::{
    ...
    WorkspaceOverride, ChokkinConfig,
};
...
// parse.rs
fn parse_dot_chokkin_toml(path: &Path) -> Result<PartialConfig, ConfigError>;
fn parse_chokkin_toml(path: &Path) -> Result<PartialConfig, ConfigError>;
fn parse_pyproject_tool_chokkin(path: &Path) -> Result<(PartialConfig, Option<UvWorkspaceHint>), ConfigError>;

// defaults.rs
fn default_config() -> ChokkinConfig;
fn merge_layers(layers: &[PartialConfig]) -> ChokkinConfig;
```

`PartialConfig` は `Option` フィールドの struct で、マージ時に `Some` のみ上書きする（配列・map は丸ごと置換。例外として `plugins` / `dependencies` はキー単位マージ）。
...
load_config(root):
  ...
  2. if root/.chokkin.toml exists:
       layers.push(parse_dot_chokkin_toml(...))
  3. if root/chokkin.toml exists:
       layers.push(parse_chokkin_toml(...))
  4. uv_hint ← None
     if root/pyproject.toml exists:
       (partial, uv_hint) ← parse_pyproject_tool_chokkin(...)
       if partial has any [tool.chokkin] keys:
         layers.push(partial)
...
3. `table.get("tool").and_then(|t| t.get("chokkin"))` のサブテーブルのみ `serde` deserialize
4. `[tool.chokkin]` が **無い**場合は layer に追加しない（defaults のみ）
...
| `entry` に `:` 複数で symbol 解釈不能（path 内 `:` 残留含む） | `Validation` |
| `entry` の symbol が空（`manage.py:`） | `Validation` |
| `ignore` の key が `CHK00[0-9]` / `CHK010` 以外 | `Validation` |
| `plugins` / `dependencies` / `workspaces.<id>` に未知キー | `UnknownKey` |
| 空の `workspace.path` | `Validation` |

バリデーションは **パース時にレイヤー単位** で行い、エラーの `path` は違反値を実際に含むファイルを指す（マージ後検証だと帰属が曖昧になるため）。
...
| シナリオ | 期待結果 |
| --- | --- |
| 設定ファイルが一切無い | `DEFAULTS` の `ChokkinConfig`、`sources.used_defaults = true` |
| `[tool.chokkin]` が空テーブル `{}` | defaults と同等（空テーブルは layer 追加しない扱いでも可。挙動は defaults と同一であること） |
| `pyproject.toml` に `[tool.chokkin]` も `[tool]` 他セクションのみ | `[tool.chokkin]` 無し → standalone / defaults のみ |
| `chokkin.toml` と `[tool.chokkin]` の両方 | pyproject 側が `mode` 等を上書き |
...
| root が `RequirementsTxt` marker のみ | `chokkin.toml` / `.chokkin.toml` は読む。`uv_workspace` は常に `None` |
| `[tool.chokkin.ignore]` に `CHK099` | `Validation` |
| `[tool.chokkin.plugins]` に `unknown = true` | `UnknownKey` |
| `[tool.chokkin.plugins]` に `celery = true` のみ | defaults に overlay（pytest 等は有効のまま） |
| `[tool.chokkin.dependencies]` に `dev_groups` のみ | 他 2 キーは defaults を維持 |
...
tests/fixtures/config/
  ...
  pyproject_full/            # [tool.chokkin] 全フィールド例
  chokkin_toml_only/           # chokkin.toml のみ
  dot_chokkin_only/            # .chokkin.toml のみ
  merge_priority/            # 3 ソース併存、優先順位検証
  workspace_overrides/       # [tool.chokkin.workspaces.*]
...
| T2 | `loads_pyproject_tool_chokkin` | 全フィールド deserialize |
...
`serde` の `deny_unknown_fields` を `RawToolChokkin` に付与し、未知トップレベルキーを `UnknownKey` に変換する。
...
- [x] `[tool.chokkin]` の §5 掲載フィールドを deserialize できる
...
3. config/types.rs — ChokkinConfig, EntrySpec, enums
...
所要: 新規 Rust ファイル 7、テストフィクスチャ 10 前後、依存 2 crate（`toml`, `serde`）。Step 3 (manifest) とは `pyproject.toml` を両方読むが、責務は `[tool.chokkin]` vs `[project]` で分離する。
...
| `chokkin.toml` と `.chokkin.toml` の優先順位をユーザー設定で入れ替え | 需要低 | issue 化 |
| `[tool.chokkin]` 以外の Knip 互換 standalone のみ運用 | pyproject 第一候補で足りる | フィードバック後 |
...
| Phase 0 exit（`uvx chokkin` 動作） | Step 2 単体では未達。Step 3 縦スライスで manifest 表示まで |
...
**合格 — 実装着手可。** Step 2 は Step 1 の `ProjectRoot` にのみ依存し、Step 3 以降へ `ChokkinConfig` を渡す縦スライスの第 2 層として独立して並行実装可能。
