...
> [!WARNING]
> **Status: pre-alpha。** `yokei` を実行すると **probe モード**が動き、パイプライン ステップ 1–4（discovery、config、manifest、sources）を実行してプロジェクト概要を表示します。ステップ 5–6（`src/plugins/` の config/plugin extraction と `src/parser/` の Python parse）はライブラリ API として利用可能です。`parse_file` / `parse_project_sources` は import・symbol・`__all__`・ignore 指令を抽出し、到達性グラフの入力を供給します。issue 報告（ステップ 7–12）は CLI には未接続です。Phase 0 では `src/graph/` の骨格ノードと import 辺も追加済みです。以下のコマンド出力は解析完了後の**目標** UX です。仕様は [`docs/dev/spec.ja.md`](./docs/dev/spec.ja.md) を参照してください。
