...
> [!WARNING]
> **Status: pre-alpha。** `yokei` を実行すると **probe モード**が動き、パイプライン ステップ 1–4（discovery、config、manifest、sources）を実行してプロジェクト概要を表示します。ステップ 5–9（`plugins`、`parser`、`resolver`、`entry`、`reachability`）はライブラリ API として利用可能です — `analyze_reachability` による entry root からのファイル到達性解析と `trace_to_file` による import 経路の追跡を含みます。issue 報告（ステップ 10–12）は CLI には未接続です。
