...
- `--fix` — 確実なdependency findingに対して保守的な修正を適用します。`--allow-remove-files` を追加すると、確実に未到達なファイルも削除対象にします。`--add-missing` は将来の missing dependency 追加用に認識されますが、現時点では skipped fix として報告します。
