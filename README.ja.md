...
> [!WARNING]
> **Status: pre-alpha。** `yokei` を実行すると **probe モード**が動き、パイプライン ステップ 1–4（discovery、config、manifest、sources）を実行してプロジェクト概要を表示します。ステップ 5–13 はライブラリ API として利用可能です — `analyze_reachability`、`trace_to_file`、`reconcile_dependencies`、`analyze_symbols` に加え、ignore / confidence フィルタと exit code 判定を行う `emit_issues`（YOK001–YOK010）、および certain な YOK002 / YOK005 / YOK009 向け manifest 自動編集の `apply_fixes` を含みます。フル分析 CLI と reporter 描画は未接続です。
