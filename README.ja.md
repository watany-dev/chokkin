...
> [!WARNING]
> **Status: pre-alpha。** `yokei` を実行すると **probe モード**が動き、パイプライン ステップ 1–4（discovery、config、manifest、sources）を実行してプロジェクト概要を表示します。ステップ 5–11（`plugins`、`parser`、`resolver`、`entry`、`reachability`、`rules`）はライブラリ API として利用可能です — `analyze_reachability` による entry root からのファイル到達性解析、`trace_to_file` による import 経路の追跡、`reconcile_dependencies` による YOK002–YOK009 の依存 issue 候補生成、`analyze_symbols` による YOK006–YOK007 / YOK010 のシンボル issue 候補生成を含みます。issue 報告（ステップ 12）は CLI には未接続です。
