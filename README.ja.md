...
> [!WARNING]
> **Status: pre-alpha。** 処理ステップ 1–5（`src/discovery/` の project root discovery、`src/config/` の `[tool.yokei]` config load、`src/manifest/` の manifest extraction、`src/sources/` の source file discovery、`src/plugins/` の config/plugin extraction）はライブラリ API として実装済みです。Phase 0 で `src/graph/`（`build_graph_skeleton`、import 辺）と `src/parser/`（`parse_file`、import 抽出 spike）を追加しました。CLI 解析と issue 報告は未接続です。この README は設計済みの挙動を記述しており、仕様の全文は [`docs/dev/spec.ja.md`](./docs/dev/spec.ja.md) にあります。以下のコマンドと出力は目指す UX を示すものです。
