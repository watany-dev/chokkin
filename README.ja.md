...
> [!WARNING]
> **Status: pre-alpha。** `yokei` を実行すると **probe モード**が動き、パイプライン ステップ 1–4（discovery、config、manifest、sources）を実行してプロジェクト概要を表示します。未使用依存・ファイルの本格解析は未実装です。Phase 0 では `src/graph/` と `src/parser/` の骨格も追加済みです。以下のコマンド出力は解析完了後の**目標** UX です。仕様は [`docs/dev/spec.ja.md`](./docs/dev/spec.ja.md) を参照してください。
