...
> [!WARNING]
> **Status: pre-alpha。** `yokei` を実行すると **probe モード**が動き、パイプライン ステップ 1–4（discovery、config、manifest、sources）を実行してプロジェクト概要を表示します。ステップ 5–8（`plugins`、`parser`、`resolver`、`entry`）はライブラリ API として利用可能です — `parse_project_sources`、bundled map 付きの `resolve_imports`、到達性解析用の `build_entry_roots` を含みます。issue 報告（ステップ 9–12）は CLI には未接続です。
