...
> [!WARNING]
> **Status: pre-alpha.** Running `yokei` executes **probe mode**: pipeline steps 1–4 (discovery, config, manifest, sources) and prints a project summary. Steps 5–10 (`plugins`, `parser`, `resolver`, `entry`, `reachability`, `rules`) are available as library APIs — including `analyze_reachability` for entry-root file reachability, `trace_to_file` for import chains, and `reconcile_dependencies` for YOK002–YOK009 dependency issue candidates. Full issue reporting (steps 11–12) is not wired to the CLI yet.
