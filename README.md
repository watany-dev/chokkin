...
> [!WARNING]
> **Status: pre-alpha.** Pipeline steps 1–3 (`src/discovery/` project root discovery, `src/config/` `[tool.yokei]` config load, and `src/manifest/` manifest extraction) are implemented as library APIs; the CLI analyzer and issue reporting are not wired yet. This README describes the designed behavior; the full specification lives in [`docs/dev/spec.ja.md`](./docs/dev/spec.ja.md). Commands and output below show the target UX.
