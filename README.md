...
- `--fix` — apply conservative fixes for certain dependency findings; add `--allow-remove-files` to also remove certain unreachable files. `--add-missing` is recognized for future missing-dependency insertion and currently reports skipped fixes.
