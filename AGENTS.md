...
run them when touching hot paths. `pipeline` also covers full analysis, cold/disk-warm parse,
reachability cache on/off, and discovery after cache population with ~2 KiB modules.
It defaults to 1k modules; set `CHOKKIN_BENCH_LARGE=1` for 5k/10k as well:
