...
pub fn collect_dynamic_imports(stmts: &[Stmt], out: &mut Vec<DynamicImport>, opaque: &mut bool) {
    for stmt in stmts {
        collect_from_stmt(stmt, out, opaque);
    }
}

#[allow(clippy::too_many_lines)]
fn collect_from_stmt(stmt: &Stmt, out: &mut Vec<DynamicImport>, opaque: &mut bool) {
    match stmt {
        Stmt::FunctionDef(function) => {
            for inner in &function.body {
                collect_from_stmt(inner, out, opaque);
            }
        },
        ...
        Stmt::Expr(expr_stmt) => {
            inspect_expr(&expr_stmt.value, out, opaque);
        },
        _ => {},
    }
}

fn inspect_expr(expr: &Expr, out: &mut Vec<DynamicImport>, opaque: &mut bool) {
    if let Expr::Call(call) = expr {
        if let Some(module) = extract_literal_module_call(&call.func, &call.args) {
            let line = call.range.start().to_u32().saturating_add(1);
...
mod tests {
    ...
    fn extracts_importlib_literal() {
        ...
        collect_dynamic_imports(&stmts, &mut imports, &mut opaque);
