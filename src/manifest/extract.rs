...
fn merge_metadata(mut base: ProjectMetadata, overlay: ProjectMetadata) -> ProjectMetadata {
    ...
    if overlay.requires_python.is_some() {
        base.requires_python = overlay.requires_python;
    }
    if !overlay.dynamic.is_empty() {
        base.dynamic = overlay.dynamic;
    }
    base
}
